...
#$URL='http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.$YYYY$MM$DD$HH/gfs.t${HH}z.pgrb2f${FHR}';
# 12-km NAM
#$URL='http://nomads.ncep.noaa.gov/pub/data/nccf/com/nam/prod/nam.$YYYY$MM$DD/nam.t${HH}z.conusnest.hiresf${FHR}.tm00.grib2';
# NARR
#$URL='https://nomads.ncdc.noaa.gov/data/narr/$YYYY$MM/$YYYY$MM$DD/narr-a_221_$YYYY$MM$DD_$HH00_000';
## RAP-130
$URL='https://nomads.ncdc.noaa.gov/data/rucanl/$YYYY$MM/$YYYY$MM$DD/rap_130_$YYYY$MM$DD_$HH00_000';
#
...
#RSS   if ($fhr <= 9) { $fhr="0$fhr"; }
   ...
   if ($range ne '') {
               print "getting $url";
      $err = system("$curl -f -s -r \"$range\" $url$grb -o $OUTDIR/$file.tmp");
      $err = $err >> 8;
      if ($err != 0) {
      ...
      rename "$OUTDIR/$file.tmp", "$OUTDIR/$file$grb";
      $output = "$output $OUTDIR/$file$grb";
   }
   else {
